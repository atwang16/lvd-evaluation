...
void detectAndCompute(string descriptor, string parameter_file, cv::Mat image, vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors,
	...
	while(getline(params, line)) {
		...
		else if(var == "RESPONSE_THRESHOLD") {
			response_threshold = stoi(value);
		}
		else if(var == "LINE_THRESHOLD_PROJECTED") {
			line_threshold_projected = stoi(value);
		}
		else if(var == "LINE_THRESHOLD_BINARIZED") {
			line_threshold_binarized = stoi(value);
		}
		else if(var == "SUPPRESS_NONMAX_SIZE") {
