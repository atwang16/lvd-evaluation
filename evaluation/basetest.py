...
def generate_descriptors(desc, img_db, det_db):
    global descriptor_executable, descriptor_parameters_path, image_db_path, detector_db_path, destination_path, overwrite_flag
    args = [descriptor_executable, desc, descriptor_parameters_path, image_db_path, detector_db_path, destination_path, overwrite_flag]
...
def generate_results(desc, sequence):
    global basetest_executable, image_db_path, results_ds_path, results_kp_path, file_output
    img_num = 2
...
if __name__ == "__main__":
    ...
    if desc_name.find("_"):
        det_desc_split = desc_name.split("_")
        det_name = det_desc_split[0]
        desc_name = det_desc_split[1]
    else:
        det_name = desc_name
    ...
    while arg_index < len(sys.argv):
        if sys.argv[arg_index] == "-generate_descriptors":
            generate_descs = True
        if sys.argv[arg_index] == "-results_only" and not(generate_descs):
            results_only = True
        arg_index += 1
    ...
    results_ds_path = os.path.join(results_path, desc_name, database)
    results_kp_path = os.path.join(results_path, det_name, database)
    ...
    if generate_descs:
        generate_descriptors(desc_name, database, det_name)

    db_sequences = sorted(os.listdir(results_ds_path))

    file_output = os.path.join(results_ds_path, desc_name + "_" + database[0:3] + "_basetest.csv")
